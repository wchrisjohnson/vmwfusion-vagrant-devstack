...
echo "===== Adding wordpress to the /etc/hosts file"
sudo -- sh -c "echo 127.0.1.1 wordpress >> /etc/hosts"
...
sudo debconf-set-selections <<< "mysql-server-5.5 mysql-server/root_password password stack"
sudo debconf-set-selections <<< "mysql-server-5.5 mysql-server/root_password_again password stack"
...
mysqladmin -u root --password=stack create wordpress
echo "----- Database created."

echo "===== Creating wordpress mysql user..."
mysql -u root --password=stack wordpress -e "GRANT ALL PRIVILEGES ON wordpress.* TO 'wordpress'@'localhost' IDENTIFIED BY 'stack';FLUSH PRIVILEGES;"
...
sed -i 's/password_here/stack/g' /var/www/html/wp-config.php
